...
import { Users, ArrowLeft, Trophy, BookOpen } from "lucide-react";
    ...
    <div className="mx-auto max-w-3xl px-5 pt-6 animate-fadeUp">
      <header className="mb-5">
        ...
        <p className="text-ink/60 mt-1 text-sm">
          كل فريق له قصته ورسالته من الكتاب المقدس.
        </p>
      </header>
          ...
          >
            ...
            <div className="flex items-center gap-3 p-4 text-white">
              <span className="relative shrink-0">
                ...
                />
                {/* admin logo overlays the avatar corner if uploaded */}
                {logos[team.id] && (
                  <img
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-white/85 text-xs mt-0.5 flex items-center gap-1.5">
                  <BookOpen size={12} /> {team.verseRef} ·{" "}
                  {toArabicDigits(team.members.length)} عضو
                </p>
              </div>
            ...
            {/* message + CTA on team color */}
            <div className="p-4 flex items-center gap-3">
              <p className="flex-1 text-sm text-white font-semibold leading-relaxed drop-shadow">
                « {team.message} »
              </p>
              <span className="chip bg-white text-deep shrink-0 font-extrabold">
                عرض القصة <ArrowLeft size={14} />
              </span>
            </div>
          </Link>
        ))}
